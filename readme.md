...
Classifies the requested task from a natural language query.
Validates input modality (Single vs. Paired) and format (GeoTIFF).
Selects the optimal tool (e.g., ChangeFormer for change detection or Qwen2-VL for VQA).
Generates an Auditable Execution Trace containing model names, parameters, and confidence scores.
